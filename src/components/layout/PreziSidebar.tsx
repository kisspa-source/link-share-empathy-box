  ...
  return (
    <>
      ...
      {!isMobile && (
        <motion.aside
          className={cn(
            "sidebar fixed inset-y-0 left-0 z-30 border-r bg-background",
      ...
      {isMobile && isMobileMenuOpen && (
        <>
          ...
          {/* 사이드바 */}
          <motion.aside
            className="fixed inset-y-0 left-0 z-50 w-64 bg-background border-r flex flex-col h-screen pt-14 md:hidden"
            initial={{ x: -256 }}
            animate={{ x: 0 }}
            exit={{ x: -256 }}
            transition={{ duration: 0.3, ease: "easeInOut" }}
